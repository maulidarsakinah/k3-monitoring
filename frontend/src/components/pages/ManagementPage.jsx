...
export default function ManagementPage({ user, onChanged }) {
  ...
  const loadData = async () => {
    ...
    setMessage("");
    try {
      const [userData, cameraData, ruleData, streamData] = await Promise.all([
        getUsers(),
        getCameras(),
        getRules(),
      ...
      setRules(ruleData);
      setStreamStatuses(Array.isArray(streamData) ? streamData : []);
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {APD_OPTIONS.map((apd) => (
                                  <label key={apd} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600">
                                    <input
                                      type="checkbox"
                                      checked={ruleEditForm.required_apd.includes(apd)}
                                      onChange={(e) => {
