...
def _append_common_filters(where, params, camera_id=None, severity=None):
    if camera_id:
        where += " AND camera_id = ?"
...
def get_trend(start_date=None, end_date=None, camera_id=None, severity=None, date_range=None) -> list[dict]:
    where, params = _build_date_filter(start_date, end_date, date_range)
    where, params = _append_common_filters(where, params, camera_id, severity)
    query = f"""
        SELECT
    ...
    return [
        {
            ...
            "approved": row[3] or 0,
            "rejected": row[4] or 0,
            "staff_reviewed": row[5] or 0,
            "needs_manager": row[6] or 0,
        }
        for row in rows
    ]


def get_distribution(start_date=None, end_date=None, camera_id=None, severity=None, date_range=None) -> dict:
    where, params = _build_date_filter(start_date, end_date, date_range)
    where, params = _append_common_filters(where, params, camera_id, severity)
    with _get_conn() as conn:
...
def get_heatmap(start_date=None, end_date=None, camera_id=None, severity=None, date_range=None) -> dict:
    where, params = _build_date_filter(start_date, end_date, date_range)
    where, params = _append_common_filters(where, params, camera_id, severity)
    query = f"""
...
def get_camera_breakdown(start_date=None, end_date=None, camera_id=None, severity=None, date_range=None) -> list[dict]:
    where, params = _build_date_filter(start_date, end_date, date_range)
    where, params = _append_common_filters(where, params, camera_id, severity)
    query = f"""
